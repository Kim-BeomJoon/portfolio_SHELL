...
# 출력 파일 경로 설정
OUTPUT_FILE="/tmp/k8s_worker/k8s_W_AWS_secuwow_$(date +%Y%m%d).txt"
...
print_logo() {
    ...
    echo "       진단일시: $(date +%Y-%m-%d)" | tee -a "$OUTPUT_FILE"
...
check_iam_role() {
    ...
    detail+="[ 현재 설정 상태 ]\n"
    detail+="# curl -s http://169.254.169.254/latest/meta-data/iam/security-credentials/\n"
    
    # IAM 역할 확인
    local iam_role=$(curl -s http://169.254.169.254/latest/meta-data/iam/security-credentials/ 2>/dev/null)
    if [ $? -eq 0 ] && [ ! -z "$iam_role" ]; then
        detail+="$iam_role\n\n"
        
        # IAM 역할의 권한 확인
        local role_arn=$(aws sts get-caller-identity --query 'Arn' --output text 2>/dev/null)
        if [ $? -eq 0 ] && [ ! -z "$role_arn" ]; then
            detail+="# aws sts get-caller-identity\n"
            detail+="$role_arn\n\n"
            
            # IAM 정책 확인
            local policies=$(aws iam list-attached-role-policies --role-name "$iam_role" 2>/dev/null)
            if [ $? -eq 0 ]; then
                detail+="# aws iam list-attached-role-policies --role-name $iam_role\n"
                detail+="$policies\n"
            fi
        else
            is_secure=false
    ...
    if [ "$is_secure" = false ]; then
        detail+="[문제점]\n"
        if [ -z "$iam_role" ]; then
            detail+="- IAM 역할이 설정되어 있지 않음\n"
        fi
        if [ -z "$role_arn" ]; then
            detail+="- IAM 역할의 ARN을 가져올 수 없음\n"
        fi

        recommendation="[조치 방법]\n"
        recommendation+="1. 적절한 IAM 역할 생성:\n"
        recommendation+="   $ aws iam create-role --role-name k8s-worker-role --assume-role-policy-document file://trust-policy.json\n\n"
        recommendation+="2. 필요한 정책 연결:\n"
        recommendation+="   $ aws iam attach-role-policy --role-name k8s-worker-role --policy-arn arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy\n\n"
        recommendation+="3. 인스턴스 프로필 생성 및 연결:\n"
        recommendation+="   $ aws iam create-instance-profile --instance-profile-name k8s-worker-profile\n"
        recommendation+="   $ aws iam add-role-to-instance-profile --instance-profile-name k8s-worker-profile --role-name k8s-worker-role\n\n"
        recommendation+="[보안 위험]\n"
        recommendation+="1. IAM 역할이 없으면 필요한 AWS 서비스에 접근할 수 없습니다.\n"
        recommendation+="2. 과도한 권한이 부여된 IAM 역할은 보안 위험을 초래할 수 있습니다."
    fi

    if [ "$is_secure" = true ]; then
        print_result "IAM 역할 설정" "양호" \
            "IAM 역할이 적절하게 설정되어 있습니다.\n\n$detail" \
            "$recommendation"
    else
        print_result "IAM 역할 설정" "취약" \
            "IAM 역할이 적절하게 설정되어 있지 않습니다.\n\n$detail" \
            "$recommendation"
    fi
}
...
check_security_groups() {
    ...
    detail+="[ 현재 설정 상태 ]\n"
    detail+="# aws ec2 describe-security-groups\n"
    
    # 인스턴스 ID 확인
    local instance_id=$(curl -s http://169.254.169.254/latest/meta-data/instance-id)
    if [ ! -z "$instance_id" ]; then
        # 인스턴스의 보안 그룹 확인
        local security_groups=$(aws ec2 describe-security-groups --filters "Name=group-name,Values=*" --query 'SecurityGroups[*].[GroupId,GroupName,IpPermissions]' --output text 2>/dev/null)
        if [ $? -eq 0 ]; then
            detail+="$security_groups\n\n"
            
            # 보안 그룹 규칙 분석
            if echo "$security_groups" | grep -q "0.0.0.0/0"; then
                is_secure=false
            fi
        else
            is_secure=false
    ...
    if [ "$is_secure" = false ]; then
        detail+="[문제점]\n"
        if [ -z "$instance_id" ]; then
            detail+="- 인스턴스 ID를 가져올 수 없음\n"
        fi
        if echo "$security_groups" | grep -q "0.0.0.0/0"; then
            detail+="- 모든 IP(0.0.0.0/0)에 대한 접근이 허용됨\n"
        fi

        recommendation="[조치 방법]\n"
        recommendation+="1. 보안 그룹 규칙 검토 및 수정:\n"
        recommendation+="   $ aws ec2 describe-security-groups --group-ids [보안그룹ID]\n"
        recommendation+="   $ aws ec2 revoke-security-group-ingress --group-id [보안그룹ID] --protocol tcp --port [포트번호] --cidr 0.0.0.0/0\n\n"
        recommendation+="2. 필요한 포트만 허용:\n"
        recommendation+="   - Kubernetes API 서버 (443)\n"
        recommendation+="   - NodePorts (30000-32767)\n"
        recommendation+="   - 필요한 서비스 포트\n\n"
        recommendation+="[보안 위험]\n"
        recommendation+="1. 모든 IP에 대한 접근이 허용되면 무단 접근의 위험이 있습니다.\n"
        recommendation+="2. 불필요한 포트가 열려있으면 공격 표면이 증가합니다."
    fi

    if [ "$is_secure" = true ]; then
        print_result "보안 그룹 설정" "양호" \
            "보안 그룹이 적절하게 설정되어 있습니다.\n\n$detail" \
            "$recommendation"
    else
        print_result "보안 그룹 설정" "취약" \
            "보안 그룹 설정이 취약합니다.\n\n$detail" \
            "$recommendation"
    fi
}
...
check_ebs_encryption() {
    ...
    detail+="[ 현재 설정 상태 ]\n"
    detail+="# aws ec2 describe-volumes\n"
    
    # 인스턴스에 연결된 EBS 볼륨 확인
    local volumes=$(aws ec2 describe-volumes --filters "Name=attachment.instance-id,Values=$(curl -s http://169.254.169.254/latest/meta-data/instance-id)" --query 'Volumes[*].[VolumeId,Encrypted]' --output text 2>/dev/null)
    if [ $? -eq 0 ]; then
        detail+="$volumes\n\n"
        
        # 암호화되지 않은 볼륨 확인
        if echo "$volumes" | grep -q "False"; then
            is_secure=false
        fi
    else
        is_secure=false
    ...
    if [ "$is_secure" = false ]; then
        detail+="[문제점]\n"
        if [ -z "$volumes" ]; then
            detail+="- EBS 볼륨 정보를 가져올 수 없음\n"
        fi
        if echo "$volumes" | grep -q "False"; then
            detail+="- 암호화되지 않은 EBS 볼륨이 존재함\n"
        fi

        recommendation="[조치 방법]\n"
        recommendation+="1. 기존 볼륨 암호화:\n"
        recommendation+="   $ aws ec2 create-snapshot --volume-id [볼륨ID]\n"
        recommendation+="   $ aws ec2 copy-snapshot --source-region [리전] --source-snapshot-id [스냅샷ID] --encrypted\n"
        recommendation+="   $ aws ec2 create-volume --availability-zone [가용영역] --encrypted --size [크기] --snapshot-id [암호화된스냅샷ID]\n\n"
        recommendation+="2. 새로운 볼륨 생성 시 암호화 활성화:\n"
        recommendation+="   $ aws ec2 create-volume --availability-zone [가용영역] --encrypted --size [크기]\n\n"
        recommendation+="[보안 위험]\n"
        recommendation+="1. 암호화되지 않은 데이터는 도난 위험이 있습니다.\n"
        recommendation+="2. 규정 준수(Compliance) 요구사항을 충족하지 못할 수 있습니다."
    fi

    if [ "$is_secure" = true ]; then
        print_result "EBS 볼륨 암호화" "양호" \
            "모든 EBS 볼륨이 암호화되어 있습니다.\n\n$detail" \
            "$recommendation"
    else
        print_result "EBS 볼륨 암호화" "취약" \
            "암호화되지 않은 EBS 볼륨이 존재합니다.\n\n$detail" \
            "$recommendation"
    fi
}
...
check_version_update() {
    ...
    detail+="[ 현재 설정 상태 ]\n"
    detail+="# yum update-minimal --security\n"
    
    # 패치 관리 프로그램 설치 여부 확인
    if command -v yum &> /dev/null; then
        local installed_packages=$(yum check-update --security 2>/dev/null | grep -E "^\s*([0-9]+)\s" | awk '{print $1}')
        if [ -z "$installed_packages" ]; then
            detail+="최신 보안 패치가 적용되어 있습니다.\n\n"
        else
            detail+="최신 보안 패치가 적용되지 않은 패키가 있습니다.\n\n"
            detail+="$installed_packages\n"
        fi
    else
        is_secure=false
    fi
    ...
    if [ "$is_secure" = false ]; then
        detail+="[문제점]\n"
        if [ -z "$installed_packages" ]; then
            detail+="- 최신 보안 패치가 적용되어 있지 않음\n"
        fi
        if [ ! -z "$installed_packages" ]; then
            detail+="- 최신 보안 패치가 적용되지 않은 패키가 있음\n"
        fi

        recommendation="[조치 방법]\n"
        recommendation+="1. 최신 보안 패치 적용:\n"
        recommendation+="   $ yum update-minimal --security\n\n"
        recommendation+="[보안 위험]\n"
        recommendation+="1. 보안 취약점이 있는 패키를 사용하면 보안 위험이 있습니다.\n"
        recommendation+="2. 규정 준수(Compliance) 요구사항을 충족하지 못할 수 있습니다."
    fi
...
main() {
    print_logo
    print_output "${BOLD}[Kubernetes AWS Worker 노드 보안 진단 시작]${NC}\n" \
                "[Kubernetes AWS Worker 노드 보안 진단 시작]"
    
    # 기본 Kubelet Configuration 검사
    ...
    # AWS 특화 보안 설정 검사
    print_output "\n${BLUE}[KuW-AWS-01] IAM 역할 설정${NC}" \
                "\n[KuW-AWS-01] IAM 역할 설정"
    check_iam_role
    
    print_output "\n${BLUE}[KuW-AWS-02] 보안 그룹 설정${NC}" \
                "\n[KuW-AWS-02] 보안 그룹 설정"
    check_security_groups
    
    print_output "\n${BLUE}[KuW-AWS-03] EBS 볼륨 암호화${NC}" \
                "\n[KuW-AWS-03] EBS 볼륨 암호화"
    check_ebs_encryption
